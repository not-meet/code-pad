...
export const deleteSnippet = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (snippet.userId !== identity.subject) {
      throw new Error('authority to delete the snippet denied');
    }
...
export const isSnippetStarred = query({
  ...
  handler: async (ctx, args) => {
    ...
    const star = await ctx.db
      ...
      .filter((q) =>
        q.eq(q.field('userId'),
          identity.subject) && q.eq(q.field('snippetId'), args.snippetId))
      .first()
...
export const getSnippetsById = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!snippet) throw new Error('no snippet of this Id found');
